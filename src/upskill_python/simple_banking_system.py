    ...
    def change_balance_window():
        def deposit():
            ...
            try:
                ...
                messagebox.showinfo(
                    ...
                    message=f"{client_name} account was deposed. Current balance: {client.balance}",
                    parent=new_window,
                )
                new_window.destroy()
            except ValueError as e:
                messagebox.showinfo(title="Operation failed.", message=f"{e}", parent=new_window)
                new_window.destroy()
        ...
        def withdraw():
            ...
            try:
                ...
                messagebox.showinfo(
                    title="Operation completed",
                    message=f"{client_name} account was withdrawn. Current balance: {client.balance}",
                    parent=new_window,
                )
                new_window.destroy()
            except ValueError as e:
...
if __name__ == "__main__":
    ...
    label = tk.Label(
        root,
        text="Number of clients in the bank: " + str(len(bank_instance.client_list)),
        font=("Arial", 17),
        bg="#9DC183",
    )
    ...
    add_client_button = tk.Button(
        root, text="Add Client", command=add_client_window, fg="black", bg="#8bc34a", width=20, height=7
    )
    add_client_button.pack(padx=5, fill="x")
    all_clients_button = tk.Button(
        root, text="Show All Clients", command=show_all_clients, fg="black", bg="#8bc34a", width=20, height=7
    )
    all_clients_button.pack(padx=5, fill="x")
    existing_client_button = tk.Button(
        root, text="Check existing client", command=existing_client_window, fg="black", bg="#8bc34a", width=20, height=7
    )
